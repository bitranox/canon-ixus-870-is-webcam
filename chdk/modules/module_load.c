...
// Last module load error (accessible from PTP handler for diagnostics)
char module_last_error[100] = {0};
...
static void moduleload_error(const char* name, const char* text)
{
    ...
    // Save error for PTP diagnostics
    sprintf(module_last_error, "%.30s: %.60s", name, text);
