...
static void print_usage(const char* prog) {
    fprintf(stderr,
        ...
        "Streams video + audio from Canon IXUS 870 IS (CHDK) as a virtual webcam.\n"
        "\n"
        "Usage: %s [options]\n"
        "\n"
        "Streaming options:\n"
        ...
        "  --no-webcam            Skip virtual webcam (\"CHDK Webcam\") creation\n"
        "  --no-preview           Skip preview window\n"
        "  --no-decode            Skip H.264 decode (measure raw PTP throughput)\n"
        "  --timeout N            Exit after N seconds (graceful shutdown)\n"
        "\n"
        "Audio options:\n"
        "  --audio-out            Play camera mic audio through default speakers (WASAPI)\n"
        "  --audio-device NAME    Route audio to specific output device (substring match)\n"
        "                         Example: --audio-device \"VB-Audio\" for virtual cable\n"
        "\n"
        "Recording options:\n"
        "  --record FILE          Record video + audio to MKV file\n"
        "  --dump-frames DIR      Save raw H.264 frames to DIR (for analysis)\n"
        "\n"
        "Camera file management:\n"
        "  --upload LOCAL REMOTE  Upload file to camera SD card (repeatable)\n"
        "  --download REMOTE LOCAL  Download file from camera\n"
        "  --delete PATH          Delete file on camera\n"
        "  --ls PATH              List directory on camera\n"
        "  --exec SCRIPT          Execute Lua script on camera\n"
        "  --reboot               Reboot camera (use after --upload)\n"
        "\n"
        "Debug options:\n"
        ...
        "\n"
        "Examples:\n"
        "  %s                                    Stream with preview + virtual webcam\n"
        "  %s --audio-device \"VB-Audio\"           Stream with audio to virtual cable\n"
        "  %s --record meeting.mkv               Record video + audio to file\n"
        "  %s --timeout 60 --no-webcam            Preview only, 60s timeout\n"
        "  %s --ls A/DCIM/100CANON               List files on camera\n"
        "  %s --upload fw.bin A/DISKBOOT.BIN --reboot  Deploy new firmware\n"
        "\n"
        "Audio in conferencing apps (Zoom, Teams, OBS):\n"
        "  1. Install VB-Audio Virtual Cable (https://vb-audio.com/Cable/)\n"
        "  2. Run: %s --audio-device \"VB-Audio\"\n"
        "  3. In app: Video = \"CHDK Webcam\", Microphone = \"CABLE Output\"\n"
